...
public class OAPFromObservableGenerator(ITestOutputHelper output) : TestBase(output)
{
    ...
    public Task FromObservableProp()
    {
        ...
        var driver = TestHelper.TestPass<ObservableAsPropertyFromObservableGenerator>(sourceCode, d => d.Id.Equals("CS0122"));

        // Assert: Verify the generated code.
        return Verify(driver).UseDirectory(TestHelper.VerifiedFilePath<ObservableAsPropertyAttribute>());
    }
