...
public sealed partial class RoutedControlHostGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        IncrementalValuesProvider<(HierarchyInfo Hierarchy, Result<RoutedControlHostInfo> Info)> rchInfoWithErrors =
            ...
            .ForAttributeWithMetadataName(
                ...
                static (context, token) =>
                {
                    ...
                    RoutedControlHostInfo rchInfo = default!;
                    ...
                    if (context.TargetNode is ClassDeclarationSyntax declaredClass && declaredClass.Modifiers.Any(SyntaxKind.PartialKeyword))
                    {
                        ...
                        if (symbol.TryGetAttributeWithFullyQualifiedMetadataName(AttributeDefinitions.RoutedControlHostAttributeType, out var attributeData))
                        {
                            ...
                            if (constructorArgument is string viewModelTypeName)
                            {
                                ...
                                rchInfo = new RoutedControlHostInfo(
                                    classNamespace!,
                                    className,
                                    viewModelTypeName!,
                                    declaredClass,
                    ...
                    return (Hierarchy: hierarchy, new Result<RoutedControlHostInfo?>(rchInfo, diagnostics));
                })
        ...
        context.RegisterSourceOutput(rchInfo, static (context, item) =>
            context.AddSource($"{item.Hierarchy.FilenameHint}.RoutedControlHost.g.cs", Execute.GetRoutedControlHost(item.Info.Value)));
    }
